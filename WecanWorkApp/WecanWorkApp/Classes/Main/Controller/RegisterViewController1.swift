...
import IDCardCamera
import ObjectMapper
class RegisterViewController1: BaseViewController ,CardDetectionViewControllerDelegate,UITextFieldDelegate{
        ...
        RxAlamofire.requestJSON(.get, USER_CAP,parameters:parameters)
            ...
            .subscribe(onNext: { (resultEntity: ResultEntity) in
                ...
                if resultEntity.resultstatus == 0 {
                    self.timer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(RegisterViewController1.tickDown), userInfo: nil, repeats: true)
                }
            },onError: { error in
                self.enable = true
...
//        if photos.count<2||photos.contains("") {
...
//            SwiftNotice.showText("请填写用户名")
        ...
        if !self.pwd.text!.isFormat("[A-Za-z]{1}[A-Za-z0-9]{7,14}") {
//            SwiftNotice.showText("密码格式不符合请重新输入")
            self.view.makeToast("密码格式不符合请重新输入", position: .center)
            return
        }
...
//            SwiftNotice.showText("请填写正确的手机号")
        ...
        if self.yzm.text == ""{
//            SwiftNotice.showText("请填写验证码")
            self.view.makeToast("请填写验证码", position: .center)
            return
        }
        print(self.gys.text!)
        upload()
    ...
    override func viewDidLoad() {
        ...
        zd.delegate = self
        sfzNum.delegate = self
        pwd.delegate = self
        pwdAgain.delegate = self
        yzm.delegate = self
        phone.delegate = self
        uname.delegate = self
...
//        navigationItem.rightBarButtonItem = searchItem
    ...
    override func keyBoardShow() {
        mScrollView.contentInset.bottom = 380
...
extension RegisterViewController1:SearchTextFieldDelegate{
    func didSelect(_ textField: SearchTextField, _ item: ListItem) {
        gys.text = item.name
    }
}

extension RegisterViewController1:LBXScanViewControllerDelegate{
